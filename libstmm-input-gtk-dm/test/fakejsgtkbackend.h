...
{
public:
	explicit FakeGtkBackend(::stmi::JsGtkDeviceManager* p0Owner) noexcept
	: Private::Js::GtkBackend(p0Owner)
	{
		#ifndef NDEBG
		const std::string sError =
		#endif //NDEBUG
		Private::Js::GtkBackend::init(JsDeviceFiles{}, false);
		assert(sError.empty());
	}
	//
	...
	void simulateRemoveDevice(int32_t nDeviceId) noexcept
	{
	...
	bool simulateJsEvent(int32_t nDeviceId, const struct js_event* p0JsEvent) noexcept
	{
		const int32_t nIdx = findDeviceId(nDeviceId);
