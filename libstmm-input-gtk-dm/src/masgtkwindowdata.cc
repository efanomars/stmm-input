...
void GtkWindowData::enable(const shared_ptr<GtkAccessor>& refAccessor, MasGtkDeviceManager* p0Owner) noexcept
{
	assert(refAccessor);
...
void GtkWindowData::connect() noexcept
{
	assert(m_p0Owner != nullptr);
	Gtk::Window* p0GtkmmWindow = m_refAccessor->getGtkmmWindow();
	if (isEventClassEnabled(Event::Class{typeid(KeyEvent)}) && !m_oKeyPressConn.connected()) {
	...
	if (isEventClassEnabled(Event::Class{typeid(PointerEvent)}) && !m_oMotionNotifyConn.connected()) {
	...
	if (isEventClassEnabled(Event::Class{typeid(PointerScrollEvent)}) && !m_oScrollConn.connected()) {
	...
	if (isEventClassEnabled(Event::Class{typeid(TouchEvent)}) && !m_oTouchConn.connected()) {
...
void GtkWindowData::disconnect() noexcept
{
...
bool GtkWindowData::isAccesorWindow(GdkWindow* p0Window) const noexcept
{
	assert(m_bIsEnabled);
