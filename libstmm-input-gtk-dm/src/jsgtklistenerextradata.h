...
#ifndef STMI_JS_GTK_LISTENER_EXTRA_DATA_H
#define STMI_JS_GTK_LISTENER_EXTRA_DATA_H
...
public:
	...
	inline bool isButtonCanceled(int32_t nButton) const noexcept
	{
		return std::find(m_aCanceledButtons.begin(), m_aCanceledButtons.end(), nButton) != m_aCanceledButtons.end();
	}
	inline void setButtonCanceled(int32_t nButton) noexcept
	{
		m_aCanceledButtons.push_back(nButton);
	}
	inline bool isHatCanceled(int32_t nHat) const noexcept
	{
		return std::find(m_aCanceledHats.begin(), m_aCanceledHats.end(), nHat) != m_aCanceledHats.end();
	}
	inline void setHatCanceled(int32_t nHat) noexcept
	{
...
#endif /* STMI_JS_GTK_LISTENER_EXTRA_DATA_H */
