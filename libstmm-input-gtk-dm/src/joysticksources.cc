...
sigc::connection JoystickLifeSource::connect(const sigc::slot<bool, const std::string&, const std::string&, bool >& oSlot)
{
	if (m_nINotifyFD == -1) {
...
bool JoystickLifeSource::prepare(int& nTimeout)
{
	nTimeout = -1;
...
sigc::connection JoystickInputSource::connect(const sigc::slot<bool, const struct ::js_event*>& oSlot)
{
	return connect_generic(oSlot);
}
	...
	//}
bool JoystickInputSource::prepare(int& nTimeout)
{
	nTimeout = -1;
