...
# File:   uninstall_libstmm-input-gtk-dm.py

# Removes all files installed by the install_libstmm-input-gtk-dm.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libstmm-input-gtk-dm.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f        {}/include/stmm-input-gtk-dm".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f         {}/lib/libstmm-input-gtk-dm.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f         {}/lib/libstmm-input-gtk-dm.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f  {}/lib/pkgconfig/stmm-input-gtk-dm.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f   {}/share/doc/libstmm-input-gtk-dm".format(sSudo, sInstallDir).split())
		#subprocess.check_call("{} rm -r -f   {}/share/doc/libstmm-input-doc".format(sSudo, sInstallDir).split())

	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libstmm-input-gtk-dm/build".format(sSudo).split())
		subprocess.check_call("{} rm -r -f libstmm-input-gtk-dm/examples/bare-app/build".format(sSudo).split())
		subprocess.check_call("{} rm -r -f libstmm-input-gtk-dm/examples/showevs/build".format(sSudo).split())
