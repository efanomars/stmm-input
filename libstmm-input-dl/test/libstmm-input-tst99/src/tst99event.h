...
{
public:
	Tst99Event(int64_t nTimeUsec, const shared_ptr<Tst99Capability>& refTst99Capability, int32_t nButton) noexcept;

	inline int32_t getButton() const noexcept { return m_nButton; }

	inline shared_ptr<Tst99Capability> getTst99Capability() const noexcept { return m_refTst99Capability.lock(); }
	//
	shared_ptr<Capability> getCapability() const noexcept override { return m_refTst99Capability.lock(); }
	//
