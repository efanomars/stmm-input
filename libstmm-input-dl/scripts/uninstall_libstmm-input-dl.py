...
# File:   uninstall_libstmm-input-dl.py

# Removes all files installed by the install_libstmm-input-dl.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libstmm-input-dl.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f       {}/include/stmm-input-dl".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f        {}/lib/libstmm-input-dl.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f        {}/lib/libstmm-input-dl.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f {}/lib/pkgconfig/stmm-input-dl.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f  {}/share/doc/libstmm-input-dl".format(sSudo, sInstallDir).split())
			# The following is not removed because it's the place where other packages install their
			# .dlp file
		#subprocess.check_call("{} rm -r -f     {}/etc/stmm-input-dl".format(sSudo, sInstallDir).split())

	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libstmm-input-dl/build".format(sSudo).split())
