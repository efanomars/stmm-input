...
public:
	 ...
	 */
	int32_t getId() const noexcept { return m_nCapabilityId; }
	...
	public:
		 ...
		 */
		explicit Class(const std::type_info& oCapabilityType) noexcept
		 ...
		 */
		inline bool operator==(const std::type_info& oOtherCapabilityType) const noexcept
		{
		 ...
		 */
		inline bool isDeviceManagerCapability() const noexcept
		{
			return m_bIsDeviceManagerCapability;
		}
	private:
		Class(const std::type_info& oCapabilityType, bool bIsDeviceManagerCapability) noexcept
	 ...
	 */
	inline const Class& getCapabilityClass() const noexcept
	{
		return m_oClass;
	}
...
protected:
	 ...
	 */
	explicit Capability(const Class& oClass) noexcept;
	...
	public:
		 ...
		 */
		explicit RegisterClass(const char* const &p0CapabilityClassId) noexcept
		{
		 ...
		 */
		inline const Class& getCapabilityClass() noexcept
		{
...
private:
	static int32_t getNewCapabilityId() noexcept;
private:
...
public:
	/** Returns null. */
	shared_ptr<Device> getDevice() const noexcept override { return shared_ptr<Device>(); }
...
protected:
	...
	explicit DeviceManagerCapability(const Class& oClass) noexcept;
private:
...
namespace std {
	...
	{
		std::size_t operator()(const stmi::Capability::Class& oClass) const noexcept
		{
			if (!oClass) {
				return 0;
			}
