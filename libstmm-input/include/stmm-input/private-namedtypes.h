...
//std::cout << "NamedTypes: adr: " << reinterpret_cast<int64_t>(this) << '\n';
	 ...
	 */
	const char* getDataFromType(const std::type_info& oType, T& oData) const noexcept
	{
	 ...
	 */
	const std::type_info& getTypeFromStringId(const char* const &p0StringId, bool& bFound) const noexcept
	{
	...
	bool hasType(const std::type_info& oType) const noexcept
	{
		auto itFind = m_oTypeData.find(std::type_index(oType));
...
//std::cout << "NamedTypes::hasType()  type_info=" << oType.name() << " addr=" << (int64_t)&oType << "  ->" << bFound << '\n';
	 ...
	 */
	void addType(const std::type_info& oType, const char* const &p0StringId, const T& oData) noexcept
	{
//std::cout << "NamedTypes::addStringIdType()  stringId=" << p0StringId << "  type_info=" << oType.name() << " addr=" << (int64_t)&oType << std::endl;
	 ...
	 */
	void removeType(const std::type_info& oType) noexcept
	{
		auto itFind = m_oTypeData.find(std::type_index(oType));
		if (itFind == m_oTypeData.end()) {
			return;
...
private:
	...
	struct HasherStr
	{
		std::size_t operator()(const char* const p0Str) const noexcept
		{
//std::cout << "  HasherStr of " << p0Str << std::endl;
	...
	struct EqualStr
	{
		bool operator()(const char* p0Str1, const char* p0Str2) const noexcept
		{
