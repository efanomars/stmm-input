 ...
 */
using EventListener = std::function<void(const shared_ptr<Event>&)>;
...
public:
	 ...
	 */
	virtual bool operator()(const shared_ptr<const Event>& refEvent) const = 0;
protected:
	CallIf() {}
};
...
public:
	 ...
	 */
	virtual std::vector<Event::Class> getEventClasses() const = 0;
	 ...
	 */
	virtual std::vector<int32_t> getDevicesWithCapabilityClass(const Capability::Class& oCapabilityClass) const = 0;
	/** Returns the ids of all the device manager's devices.
	 * @return The set of device ids.
	 */
	virtual std::vector<int32_t> getDevices() const = 0;
	 ...
	 */
	virtual bool addAccessor(const shared_ptr<Accessor>& refAccessor) = 0;
	 ...
	 */
	virtual bool removeAccessor(const shared_ptr<Accessor>& refAccessor) = 0;
	 ...
	 */
	virtual bool hasAccessor(const shared_ptr<Accessor>& refAccessor) = 0;
