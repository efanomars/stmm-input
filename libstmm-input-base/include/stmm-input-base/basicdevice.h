...
{
public:
	std::string getName() const noexcept override { return m_sName; }
	//
	shared_ptr<DeviceManager> getDeviceManager() const noexcept override;
	//
	shared_ptr<ODM> getOwnerDeviceManager() const noexcept { return m_refOwner.lock(); }
protected:
	 ...
	 * be called.
	 */
	void resetOwnerDeviceManager() noexcept { m_refOwner.reset(); }
private:
	std::string m_sName;
...
template<class ODM>
shared_ptr<DeviceManager> BasicDevice<ODM>::getDeviceManager() const noexcept
{
