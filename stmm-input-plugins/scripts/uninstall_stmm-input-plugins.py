...
# File:   uninstall_stmm-input-plugins.py

# Removes all files installed by the install_stmm-input-plugins.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_stmm-input-plugins.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f            {}/bin/stmm-input-plugins".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f          {}/share/stmm-input-plugins".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f {}/share/man/man1/stmm-input-plugins.1.gz".format(sSudo, sInstallDir).split())

	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f stmm-input-plugins/build".format(sSudo).split())
