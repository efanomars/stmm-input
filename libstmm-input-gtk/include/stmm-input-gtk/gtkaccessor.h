...
public:
	 ...
	 */
	explicit GtkAccessor(const Glib::RefPtr<Gtk::Window>& refGtkmmWindow) noexcept;
	/** Constructs a Gtk::Window wrapper.
	 * This class keeps a weak reference to the Gtk::Window.
	 * @param p0GtkmmWindow The window. Cannot be null.
	 */
	explicit GtkAccessor(Gtk::Window* p0GtkmmWindow) noexcept;
	 ...
	 */
	inline bool isDeleted() const noexcept { return m_bIsDeleted; }
	/** The Gtk::Window pointer.
	 * To know whether the pointer is valid call isDeleted().
	 * @return The non owning Gtk::Window pointer.
	 */
	inline Gtk::Window* getGtkmmWindow() const noexcept { return m_p0GtkmmWindow; }
private:
	friend void Private::GtkAccessorNS::notify_window_destroyed(gpointer p0Data, GObject *p0Window) noexcept;
	void windowWasDestroyed(GtkWindow* p0GtkWindow) noexcept;
private:
	bool m_bIsDeleted;
