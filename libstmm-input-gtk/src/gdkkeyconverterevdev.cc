...
const shared_ptr<GdkKeyConverterEvDev>& GdkKeyConverterEvDev::getConverter() noexcept
{
	static auto s_refConverter = shared_ptr<GdkKeyConverterEvDev>(new GdkKeyConverterEvDev());
	return s_refConverter;
}

bool GdkKeyConverterEvDev::convertKeyCodeToHardwareKey(guint16 nGdkKeycode, HARDWARE_KEY& eHardwareKey) const noexcept
{
...
bool GdkKeyConverterEvDev::convertEventKeyToHardwareKey(GdkEventKey const* p0GdkEvent, HARDWARE_KEY& eHardwareKey) const noexcept
{
