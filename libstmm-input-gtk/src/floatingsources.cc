...
sigc::connection XIEventSource::connect(const sigc::slot<bool, XIDeviceEvent*>& slot)
{
	...
	if (bIsGenericEvent) {
		...
		if (XGetEventData(m_p0XDisplay, p0Cookie)) {
			if (p0Cookie->extension != m_nXiOpcode) {
				...
				XIEvent* p0XIEv = (XIEvent *) p0Cookie->data;
				const auto nEvType = p0Cookie->evtype;
				switch (nEvType) {
				case XI_KeyPress:
				case XI_KeyRelease:
				{
					XIDeviceEvent* p0XIDeviceEv = (XIDeviceEvent *) p0XIEv;
				...
				case XI_FocusOut:
				{
					XIEnterEvent* p0XIEnterEv = (XIEnterEvent *) p0XIEv;
...
{
	if (((m_oPollFd.get_revents() & G_IO_IN) == 0) || (XPending(m_p0XDisplay) <= 0)) {
