...
GtkBackend::GtkBackend(MasGtkDeviceManager* p0Owner, const Glib::RefPtr<Gdk::DeviceManager>& refGdkDeviceManager)
: m_p0Owner(p0Owner)
...
void GtkBackend::addDevices(GdkDevice* p0NotThisOne)
{
	assert(m_refGdkDeviceManager);
...
bool GtkBackend::findGdkMasterDevice(GdkDevice* p0MasterDevice, bool& bPointer) const
{
...
void GtkBackend::onDeviceChanged(bool bPointer)
{
	m_p0Owner->onDeviceChanged(bPointer);
}
void GtkBackend::onDevicePairAdded()
{
	m_p0Owner->onDevicePairAdded();
}
void GtkBackend::onDevicePairRemoved()
{
